...
#include "fix_supersaturation_mono_ext.h"
...
FixSupersaturationMonoExt::FixSupersaturationMonoExt(LAMMPS* lmp, int narg, char** arg) : Fix(lmp, narg, arg)
{
  restart_pbc = 1;
  nevery      = 1;

  if (narg < 11) { utils::missing_cmd_args(FLERR, "fix cluster/crush/delete", error); }
  ...
  if (compute_cluster_size == nullptr) { error->all(FLERR, "{}: Cannot find compute of style 'cluster/size' with id: {}", style, arg[4]); }

  compute_ss_mono = lmp->modify->get_compute_by_id(arg[5]);
  if (compute_ss_mono == nullptr) { error->all(FLERR, "{}: Cannot find compute of style 'supersaturation/mono' with id: {}", style, arg[5]); }

  // Get the desired supersaturation
  supersaturation = utils::numeric(FLERR, arg[6], true, lmp);
  if (supersaturation <= 0) { error->all(FLERR, "{}: supersaturation should be greater than 0", style); }
  ...
  const int xseed = utils::inumeric(FLERR, arg[8], true, lmp);
  xrandom         = new RanPark(lmp, xseed);

  // Get the seed for velocity generator
  const int vseed = utils::inumeric(FLERR, arg[9], true, lmp);
  vrandom         = new RanPark(lmp, vseed);

  // Get the type of atoms to create
  ntype           = utils::inumeric(FLERR, arg[10], true, lmp);
    ...
    if (::strcmp(arg[iarg], "maxtry") == 0) {
      ...
      if (::strcmp(arg[iarg + 1], "fix") == 0) {
        ...
        if (::strcmp(arg[iarg + 2], "avg") == 0) {
          temp_use_mono = false;
        } else if (::strcmp(arg[iarg + 2], "mono") == 0) {
          temp_use_mono = true;
        } else {
          error->all(FLERR, "{}: Unrecognized value of `temp track`: {}.", style, arg[iarg + 2]);
        }
      } else {
      ...
      rate_limit = utils::inumeric(FLERR, arg[iarg + 1], true, lmp);
      if (rate_limit < 0) { error->all(FLERR, "{}: Rate limit cannot be less than 0", style); }
      iarg += 2;
      ...
      iarg += 2;
    } else if (::strcmp(arg[iarg], "start_at") == 0) {
      if (iarg + 2 > narg) { utils::missing_cmd_args(FLERR, std::format("{}: start_at", style), error); }
      // Get execution period
      const bigint step = utils::inumeric(FLERR, arg[iarg + 1], true, lmp);
      if (step < 0) { error->all(FLERR, "{}: Start timestep cannot be less than 0", style); }
      next_step = step;
      iarg += 2;
  ...
  if ((comm->me == 0) && (fileflag != 0)) {
    fmt::print(fp, "ntimestep,ntotal,balance,deleted,added\n");
    ::fflush(fp);
  }
}
...
FixSupersaturationMonoExt::~FixSupersaturationMonoExt() noexcept(true)
{
...
void FixSupersaturationMonoExt::init()
{
...
int FixSupersaturationMonoExt::setmask()
{
  int mask = 0;
  mask |= PRE_EXCHANGE;
  return mask;
...
void FixSupersaturationMonoExt::pre_exchange()
{
  if (update->ntimestep < next_step) { return; }
  next_step = update->ntimestep + nevery;

  if (compute_ss_mono->invoked_scalar != update->ntimestep) { compute_ss_mono->compute_scalar(); }
  const double* const data = compute_ss_mono->vector_local;
  const int balance        = static_cast<int>(supersaturation * data[1] - data[0]);
  int deleted              = 0;
  int added                = 0;
  ...
  if (balance < 0) {
    ...
    if (to_delete > 0) {
      ...
      double* const monos = compute_cluster_size->vector_local;
      // sort to delete atoms from end to lower the number of copy opretions
      std::sort(monos, monos + compute_cluster_size->size_local_rows, std::less<>());
    ...
    if (assign_temperature && (!temp_fix)) {
      if (temp_use_mono) {
        ...
        vsigma = ::sqrt(compute_temp->vector[1] / atom->mass[ntype]);
      } else {
    ...
    added = add(rate_limit > 0 ? std::min(rate_limit, balance) : balance);
  }

  const bigint nblocal = atom->nlocal;
  ::MPI_Allreduce(&nblocal, &atom->natoms, 1, MPI_LMP_BIGINT, MPI_SUM, world);

  if (comm->me == 0) {
    // print status
    if (screenflag != 0) { utils::logmesg(lmp, "Deleted {}, added {} atoms.\n", deleted, added); }
    if (fileflag != 0) {
      utils::print(fp, "{},{},{},{},{}\n", update->ntimestep, atom->natoms, balance, deleted, added);
      ::fflush(fp);
    }
  }
...
void FixSupersaturationMonoExt::deleteAtoms(const int to_delete) const noexcept(true)
{
  // delete local atoms
  // reset nlocal
...
int FixSupersaturationMonoExt::add(const int to_insert) const
{
...
int FixSupersaturationMonoExt::placement_check_me(const std::array<double, 3>& newcoord, const double* const sublo, const double* const subhi) const
{
...
void FixSupersaturationMonoExt::create_atom(const std::array<double, 3>& coord, bigint tag) const noexcept
{
...
void FixSupersaturationMonoExt::gen_pos(std::array<double, 3>& coord) const noexcept
{
...
int FixSupersaturationMonoExt::check_overlap(const std::array<double, 3>& coord) const noexcept
{
...
void FixSupersaturationMonoExt::postDelete() const noexcept(true)
{
...
bool FixSupersaturationMonoExt::vartest(const std::array<double, 3>& coord) const noexcept
{
