...
using cluster_data = NUCC::cluster_data;
using cldata       = NUCC::cldata;
...
ComputeClusterSizeExt::ComputeClusterSizeExt(LAMMPS* lmp, int narg, char** arg) : Compute(lmp, narg, arg)
{
  ...
  if (comm->nprocs > NUCC::Defines::CLUSTER_MAX_OWNERS) {
...
void ComputeClusterSizeExt::init()
{
  ...
  counts_global.grow(memory, comm->nprocs, "size/cluster/ext:counts_global");
  displs.grow(memory, comm->nprocs, "size/cluster/ext:displs");

  dist_local.grow(memory, size_vector, "size/cluster/ext:dist_local");
  dist.grow(memory, size_vector, "size/cluster/ext:dist");
  vector = dist.data();
  ...
  nloc_gather = static_cast<bigint>(static_cast<long double>(atom->natoms) * NUCC::Defines::ALLOC_COEFF);
  gathered.grow(memory, nloc_gather, "size/cluster/ext:gathered");
  gathered.reset_unsafe<int>(0);

  nloc_peratom = static_cast<int>(atom->nlocal * NUCC::Defines::ALLOC_COEFF);
...
void ComputeClusterSizeExt::compute_vector()
{
  ...
  for (int i = 0; i < atom->nlocal; ++i) {
    if ((atom->mask[i] & groupbit) != 0) {
      ...
      if (clstr.l_size < NUCC::Defines::CLUSTER_MAX_SIZE) {
  ...
  for (int i = atom->nlocal; i < atom->nmax; ++i) {
    if ((atom->mask[i] & groupbit) != 0) {
      ...
      if (cluster_map.contains(clid)) {
        cluster_data& clstr = clusters[cluster_map[clid]];
        if (clstr.nghost < NUCC::Defines::CLUSTER_MAX_GHOST) {
  ...
  // communicate about number of unique clusters
  {
    ...
    if (to_allocate > nloc_gather) {
      nloc_gather = static_cast<bigint>(to_allocate * NUCC::Defines::ALLOC_COEFF);
      gathered.grow(memory, nloc_gather, "size/cluster/ext:gathered");
      gathered.reset_unsafe<int>(0);
    }
